...
source("./function-binned.resids.R")
...
p <-  binned.resids((subset(detWright, Year == 1)$Year+1899),
                    (subset(detWright, Year == 1))$res)
# Loop through remaining years and estimate residuals, whilsr row binding to previous
for(i in 2:106){
  p <- rbind(p, binned.resids((subset(detWright, Year == paste(i))$Year+1899),
...
plot(range(p$xbar), range(p$ybar, p$X2se, -p$X2se, na.rm=TRUE),
     xlab="Year", ylab="Binned Residuals", type="n", main="Detection: Model A all iterations") + 
  abline (0,0, lty=2) +
