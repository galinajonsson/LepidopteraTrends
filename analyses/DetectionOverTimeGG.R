...
#' @param mixLL2 optional logical argument, defaults fo ALSE and should only be set to TRUE if the mixLL2 list length specification has been used to generate model output.
#' 
#' @details 
...
DetectionOverTimeGG <- function(model, spname = NULL, min.yr = NULL, legend_labels = NULL, legend_title = NULL, mixLL2 = FALSE){
  ...
  if("LL.p" %in% names(sims_list) && "dtype2.p"  %in% names(sims_list)  && isFALSE(mixLL2)){
    # the model was fitted with a mixed list length
    pDet2 <- inv.logit(pDet1 + sims_list$dtype2.p[,1]) 
    pDet4 <- inv.logit(pDet1 + sims_list$dtype3.p[,1])
    pDet5 <- inv.logit(pDet1 + (sims_list$LL.p[,1] * log(5))) # data type 1 with list length 5
    pDet1 <- inv.logit(pDet1)
    pDet <- melt(list(pDet1, pDet2, pDet4, pDet5))
  } else if("LL.p" %in% names(sims_list) && "dtype2.p"  %in% names(sims_list)  && isTRUE(mixLL2)){
    # the model was fitted with a mixed list length but no year effect for NHC(data type 3)
    pDet2 <- inv.logit(pDet1 + sims_list$dtype2.p[,1]) 
    pDet4 <- replace(pDet1,,0)
    pDet4 <- inv.logit(pDet4 + sims_list$dtype3.p[,1])
    pDet5 <- inv.logit(pDet1 + (sims_list$LL.p[,1] * log(5))) # data type 1 with list length 5
    pDet1 <- inv.logit(pDet1)
    pDet <- melt(list(pDet1, pDet2, pDet4, pDet5))
  } else if("LL.p" %in% names(sims_list)){
    # the model was fitted with continuous list length
    pDet2 <- inv.logit(pDet1 + (sims_list$LL.p[,1] * log(2))) # list length 2
    pDet4 <- inv.logit(pDet1 + (sims_list$LL.p[,1] * log(4))) # list length 4
    pDet1 <- inv.logit(pDet1)
  ...
  #pDet <- melt(list(pDet1, pDet2, pDet4, pDet5))
  names(pDet) <- c("it", "year", "pDet", "ListLength")
