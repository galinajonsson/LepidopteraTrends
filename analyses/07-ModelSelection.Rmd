...
# Create a replica of the variable column (i.e., muZ[i,t])
#temp_data$variable <- paste(temp_data$Site, temp_data$Year, sep = ",")  
#temp_data$variable <- paste("muZ[", temp_data$variable, sep = "")  
#temp_data$variable <- as.factor(paste(temp_data$variable, "]", sep = ""))  

#length(unique(temp_data$variable))     ## 73672 unique combinations
...
# Merge with muZ and z dataframes
#muZ2 <- merge(muZ, temp_data[,-c(1,2,4)], by="variable", all.x = TRUE)
#z2 <- merge(z, temp_data[,-c(1,2,4)], by="variable", all.x = FALSE, all.y = TRUE)
#z <- merge(z[,-c(1,2,4)], temp_data)
...
**Nick**    
* I had to "rbindlist(lapply(PARAMETER, as.data.frame))" before running melt(), otherwise I got the error "*Error in vector(type, length) : vector: cannot make a vector of mode 'NULL'.*", probably because although "lapply(samp, function(x) x[, grepl("p", dimnames(x)[[2]])])" converts the mcmc.list to a 'regular' list, but the elements are still "'mcmc' num"     
* melt() gives warning: "*id.vars and measure.vars are internally guessed when both are 'NULL'. All non-numeric/integer/logical type columns are considered id.vars, which in this case are columns []. Consider providing at least one of 'id' or 'measure' vars in future.*" - this isn't an issue right as the output is what I want...    
* **BUT** I've tried three ways of adding a vector for chain number (i.e. 1,2 or 3) to the lists before melt()-ing but neither of my attempts have worked, do you reckon my shortcut for adding iteration numbers gives accurate results?     
...
#if(nrow(muZ)==nrow(z) && max(muZ$Year)==max(z$Year) && max(muZ$Site)==max(z$Site)){
#  for(i in 1:max(muZ$Year)){
#    for(j in 1:max(muZ$Site)){
#      for(n in 1:99){
#        if(i==1 && j==1 && n==1){
#          occuRes <- 
#        }
...
for(j in 1:???){
  if(i in 1:??? == 1){
    detWright + (z$z - p(z))
    detWright <- c(detResidW, (z$z[i,] - p(z[,i])))
  }
}
``` 
     
    
# NOOOOO USE BINNED RESIDUAL PLOTS!!   
## Plot
...
qqline(occuWright$res) #add a straight dia)gonal lie to the plot
