...
* ~~Run *Polygonia c-album* mixLL2 Py summary~~
* ~~Run *Polyommatus bellargus* mixLL2 Py summary~~
* ~~Run *Thecla betulae* Py mixLL2 summary~~
* Annotate shiny plotting code
* Annotate Rhat_lists function
* Plot record numbers over time
...
### Occupancy estimates
```{r plot-occu-outputs, message=FALSE, warning=FALSE, echo=FALSE}
require(shiny)
require(ggplot2)
...
```{r plot-det-outputs, message=FALSE, echo=FALSE, fig.show='hold', out.width='50%'}
require(shiny)
require(ggplot2)
require(sparta)
require(dplyr)
...
Erebia_aethiops_catLL <- catLL_list[[2]]
...
# recompile the model
Erebia_aethiops_catLL$model$recompile()

#out$model$recompile()

# Sample 100 iterations of each of the 3 chains and thin by 3, giving 99 samples
samp <- rjags:::coda.samples(model=Erebia_aethiops_catLL$model, 
...
temp_data <- Erebia_aethiops_catLL$model$data()
...
# save this file
write.csv(RR1, "../outputs/posteriorPredictiveChecks-outputs/Erebia_aethiops_catLL_RR1.csv")
...
# save this file
write.csv(RR2, "../outputs/posteriorPredictiveChecks-outputs/Erebia_aethiops_catLL_RR2.csv")
```
...
Here, I will calculate the mean of *T*~*st*~(*y*~rep~) across species for each model fomrmulation, not taxonomic group (still it calling *g*).
...
```{r variance&taxon, message=FALSE, warning=FALSE}
library("dplyr")
...
Now plot the Bayesian *p*-value (the proportion of simulated values that are lower than the observed; the 'best' Bayesian *p*-value is 0.5)
