...
The occupancy residuals, $F_{occ}$, for site $i$ is expressed as:   
...
DS detection residuals, $F_{det}$, uses the total number of detdections at a site, $Y_{i}$, 


> If the detection probability is not constant across surveys[, $F_{det}(Y_{i})$] is computed as a summation across all possible histories with $Y_{i}$ or fewer detections. The cumulative distribution function of $Y_{i}$ can be written as:

also use the posterior distribution of the occupancy state. For posterior iteration $n$, the detection residuals for site $i$ and visit $v$ is expressed as: 
$$F_{det}(Y_{i}|Y_{i}>1) =\left( \frac{1}{1-(1 - \prod_{v=1}^{V}(1-p_{in})} \right)\sum_{x|1≤n_{x}≤n_{x}}^{}) \prod_{v=1}^{V}-p_{iv}^{xv}(1-p_{iv})^{1- _{xv}}$$


```{r squirrelExample, eval=FALSE}
#source("S6residFunctions.r")
require("mgcv")

load("~/Documents/LepidopteraTrends/Resources/Warton2017/mee312761-sup-0002-datas2.rdata")

y <- as.matrix(umf@y) # Extract the observed detections and convert to binary outcomes.
s<-nrow(y)          # No. of sites (s).
K<-ncol(y)          # No. of surveys/sampling occasions (K).


 ## Label the covariates first and construct data in the RPresence format:
...
unitcov<-data.frame(int=rep(1,s),elev=elev,forest=forest)
survcov<-data.frame(int=rep(1,s*K),dur=c(dur),date=c(date))
mydata<-create.pao(data=y,unitcov=unitcov,survcov=survcov)


m1<-occ.mod(model=list(psi~elev+I(elev^2)+forest+I(forest^2)+elev:forest+elev:I(forest^2)+I(elev^2):forest,p~date+dur+I(dur^2)),data=mydata,type="so")
```
...
Here, we plot residuals according to the methodology of [Wright *et al.*, (2019)](https://esajournals.onlinelibrary.wiley.com/doi/abs/10.1002/ecy.2703). We only plot Dunn-Smyth (DS) residuals for one model (*A. urticae* model B) to demonstrate the plotting procedure, especially given that our data is formatted differently to that of Wright *et al.*, and we wish to plot residuals for multi-year occupancy models, whilst the authors used a single season example. Wright residuals for all species and models can be found under 'species-wise summary'.    
...
results_Aglais_urticae_mixLL <- readRDS("../outputs/mixLL-outputs/results_Aglais_urticae_crick_mixLL.rds")

# recompile the model
results_Aglais_urticae_mixLL$model$recompile()

# Sample 100 iterations of each of the 3 chains and thin by 3, giving 99 samples of "muZ", "z", "Py" and "p"
samp <- rjags:::coda.samples(model=results_Aglais_urticae_mixLL$model, 
                             parallel=TRUE, n.cores=3,
                             variable.names=c("muZ", "z", "p", "Py"), 
                             n.iter=100, # Define number of iterations to run
                             thin=3)
# This produced a mcmc.list of length three: one list per chain with column names in the format muZ[i,t], z[i,t], Py[j] and p[j] (where j denotes visit; v) with a total length of XXXXXXXXXXXX columns of samples estimates and 33 rows, easch, totalling 99 samples 



### Separate colums station with "muZ", "z" and "p" (will also turn the mcmc.list to a 'regular' list)
### As 'regular' lists, we can use the rehape2-function melt()
...
p <- melt(p)
Py <- lapply(samp, function(x) x[, grepl("Py", dimnames(x)[[2]])])
Py <- melt(Py)
...
# Tidy VisitID column
muZ$VisitID <- as.character(gsub(muZ$Var2, pa="muZ\\[", repl=""))
muZ$VisitID <- as.numeric(gsub(muZ$VisitID, pa="\\]", repl=""))
z$VisitID <- as.character(gsub(z$Var2, pa="z\\[", repl=""))
z$VisitID <- as.numeric(gsub(p$VisitID, pa="\\]", repl=""))
p$VisitID <- as.character(gsub(z$Var2, pa="p\\[", repl=""))
p$VisitID <- as.numeric(gsub(p$VisitID, pa="\\]", repl=""))
Py$VisitID <- as.character(gsub(z$Var2, pa="Py\\[", repl=""))
Py$VisitID <- as.numeric(gsub(p$VisitID, pa="\\]", repl=""))

# Tidy iteration
muZ$iter <- with(muZ, Var1 + max(Var1)*(L1 - 1))
z$iter <- with(z, Var1 + max(Var1)*(L1 - 1))
p$iter <- with(p, Var1 + max(Var1)*(L1 - 1))
Py$iter <- with(Py, Var1 + max(Var1)*(L1 - 1))
...
# Fanally, merge with muZ, z and p dataframes
muZ <- merge(muZ[,-c(1,2,4)], temp_data)
z <- merge(zp[,-c(1,2,4)], temp_data)
p <- merge(p[,-c(1,2,4)], temp_data)
Py <- merge(Py[,-c(1,2,4)], temp_data)
```
muZ[i,t] and z[i,t] will be loner than p[j] (all combinations of sites (?????) and year (106) vs. the total numer of visits (for the species or only focal species??????))   
...
for(j in 1:???){
  if(i in 1:??? = 1){
    detResidW + (z$z - p(z))
    detResidW <- c(detResidW, (z$z[i,] - p(z[,i])))
  }
}
``` 
...
```{r exampleWrightPlot, echo=TRUE, eval=FALSE}
par(mfrow = c(2, 2))

# Produce detection residual vs. fitted plot
plot(Py$??????, occResidW$res, 
     xlab="Occupancy $\psi_$", 
     ylab="Residuals")
abline(0,0) #add a horizontal line at 0 

# Produce occupancy residual vs. fitted plot
plot(Py$??????, detResidW$res, 
     xlab="Detection Probability $p$", 
     ylab="Residuals")
abline(0,0) #add a horizontal line at 0 


# Q-Q plot for occupancy residuals
qqnorm(occuResidW$res, 
       main="Occupancy", 
       xlab="Theoretical Quantiles", 
       ylab="Standardised Residuals")
qqline(occuResidW$res) #add a straight dia)gonal lie to the plot

# Q-Q plot for detection residuals
qqnorm(detResidW$res, 
       main="Occupancy", 
       xlab="Theoretical Quantiles", 
       ylab="Standardised Residuals")
qqline(detResidW$res) #add a straight diagonal line to the plot
```
