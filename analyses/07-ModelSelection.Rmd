...
```{r plot-PySummary, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
...
```{r plotBp, echo=FALSE, warning=FALSE, cache=TRUE}
require(tidyr)
require(viridis)
require(ggtext)
...
ggplot(ModelBp_Summ, aes(x=mixLL, y=mixLL2, group=Species)) + 
  ...
  scale_color_manual(values =c (viridis(12))) +
  scale_shape_manual(values = c(15:18, 21:24, 15:18)) + 
  ggtitle("Bayesian p-values",
          subtitle = "Mean proportion of sites with records across years") + 
  theme_bw() +
  geom_abline(aes(intercept=0, slope=1)) + 
  xlab("Model C") + ylab("Model B") +
