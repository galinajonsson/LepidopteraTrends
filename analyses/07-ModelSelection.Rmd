...
# Source function to summarise_Rhat to test whether all occupancy estimates have converged
source("./function-summarise_Rhat.R")
summarise_Rhat <- summarise_Rhat(append(catLL_list, 
                                              (append(mixLL_list,
                                              mixLL2_list))), 
                                       verbose=FALSE,
...
NotConvergedSpp <- subset(summarise_Rhat, grepl("N", converged))
...
RhatDet_mixLL <- SummariseRhatDetProb("../outputs/mixLL-outputs/", 
                              verbose=FALSE)

RhatDet_mixLL2 <- SummariseRhatDetProb("../outputs/mixLL2-outputs/", 
                              verbose=FALSE)

# If some species have not converged (converged-column == "N"), subset them
#NotConvergedDetProb_catLL <- subset(RhatDet_mixLL, grepl("N", RhatDet_catLL[,2:5]))
NotConvergedDetProb_mixLL <- subset(RhatDet_mixLL, grepl("N", RhatDet_mixLL[,2:5]))
NotConvergedDetProb_mixLL2 <- subset(RhatDet_mixLL2, grepl("N", RhatDet_mixLL2[,2:5]))
...
Pararge_aegeria_catLL <- catLL_list[[8]]
...
# recompile the model
Pararge_aegeria_catLL$model$recompile()

#out$model$recompile()

# Sample 100 iterations of each of the 3 chains and thin by 3, giving 99 samples
samp <- rjags:::coda.samples(model=Pararge_aegeria_catLL$model, 
...
temp_data <- Pararge_aegeria_catLL$model$data()
...
# save this file
write.csv(RR1, "../outputs/posteriorPredictiveChecks-outputs/Pararge_aegeria_catLL_RR1.csv")
...
# save this file
write.csv(RR2, "../outputs/posteriorPredictiveChecks-outputs/Pararge_aegeria_catLL_RR2.csv")
```
