...
# Sample 100 iterations of each of the 3 chains and thin by 3, giving 99 samples of "muZ", "z", "Py" and "y"
samp <- rjags:::coda.samples(model=results_Aglais_urticae_mixLL$model, 
                             parallel=TRUE, n.cores=3,
                             variable.names=c("muZ", "z", "p", "y"), 
                             n.iter=100, # Define number of iterations to run
                             thin=3)
# This produced a mcmc.list of length three: one list per chain with column names in the format muZ[i,t], z[i,t], y[j] and p[j] (where j denotes visit; v) with a total length of 1366187 and XXXXXXXXXXXX columns for y[j] and p[j], and uZ[i,t], z[i,t], respectively, of samples estimates and 33 rows, each, totalling 99 samples 



### Separate columns station with "muZ", "z", "Py" and "p" (will also turn the mcmc.list to a 'regular' list)
### As 'regular' lists, we can use the data.table-function melt()

# recall the format Py[j] and p[j] vs muZ[i,t] and z[i,t] (where j denotes visit)
# This makes Py and p straight forward: 
...
y <- lapply(samp, function(x) x[, grepl("y", dimnames(x)[[2]])])
y <- melt(y)
...
# Use the 99 sets of probabilities & sample a vector of simulated observations under the model, by treating each visit as a potential Bernoulli trial. Here, we use rbinom as Bernoulli trial 
Y_rep <- lapply(Py, function(x) apply(x, 1:2, rbinom, n=1, size=1))
...
p$iter <- rep(1:99, max(as.integer(p$VisitID))) 
