...
samp <- rjags:::coda.samples(model=Aglais_urticae_mixLL$model, 
                             ...
                             n.iter=100, # Define number of iterations to run
                             thin=3)
# This produced a mcmc.list of length three: one list per chain with column names in the format muZ[i,t], z[i,t] and p[j] (where j denotes visit; v) with a total length of 1366187 columns for p[j], and XXXXXXXXXXXX columns for muZ[i,t] and z[i,t], of samples estimates and 33 rows, each, totalling 99 samples 
...
muZ$Site <- as.numeric(gsub(muZ$Site, pa="\\,[:digit:]\\]", repl=""))
muZ$Year <- as.character(gsub(muZ$variable, pa="muZ\\[[:digit:]\\,", repl=""))
muZ$Year <- as.numeric(gsub(muZ$Year, pa="\\]", repl=""))
